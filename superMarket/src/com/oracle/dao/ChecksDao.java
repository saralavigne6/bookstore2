...
//					addAnnotatedClass(checks.class).buildSessionFactory();
	...
	public List<checks> searchAllNoDealChecks(){    //查询对应用户的订单
		List<checks> list = new ArrayList<checks>();
		try{
//			if(factory != null){
//				System.out.println("factory不为空");
...
//					addAnnotatedClass(checks.class).buildSessionFactory();
			...
			session = factory.openSession();
			tx = session.beginTransaction();
			String hql = "FROM checks where state = 0";
			Query query = session.createQuery(hql);
			list = (ArrayList)query.list();
		}catch(Exception e){
			e.printStackTrace();
	...
	public boolean updateDealCheck(String checkid,String manager){
		try{
			factory = new AnnotationConfiguration().configure().
					addAnnotatedClass(checks.class).buildSessionFactory();
			session = factory.openSession();
	...
	public List<checks> searchAllDealChecks(){    //查询对应用户的订单
		List<checks> list = new ArrayList<checks>();
		try{
//			if(factory != null){
//				System.out.println("factory不为空");
...
//					addAnnotatedClass(checks.class).buildSessionFactory();
			...
			session = factory.openSession();
			tx = session.beginTransaction();
			String hql = "FROM checks where state = 1";
			Query query = session.createQuery(hql);
			list = (ArrayList)query.list();
		}catch(Exception e){
			e.printStackTrace();
