...
a{
  ...
  <body>
  <c:if test="${requestScope.mess eq \"yes\" and not empty requestScope.userpass }">
  ...
  <c:if test="${requestScope.mess eq \"no\" and empty requestScope.userpass}">
  <script type="text/javascript">
  alert("账号或者电话号码不正确\n请重新输入");
  </script>
  </c:if>
  
  <c:if test="${requestScope.mess eq \"no1\" and empty requestScope.userpass }">
  <script type="text/javascript">
  alert("该用户名不存在\n请重新输入");
  </script>
  </c:if>
...
<!--/navigation -->
  ...
  <div class="content">
   ...
   <div class="login">
    <p class="head">FORGET</p>
    <form method="post" action="GetPassword">
        <input type="text" name="username" id="username" class="input1" placeholder="账号" required="" /><br>
        <input type="text" name="tel" class="input1" placeholder="手机号码" required="" /><br>
        <c:if test="${not empty requestScope.userpass }">
        <input type="text" name="userpass" class="input1" value="密码:${requestScope.userpass }" /><br>
        </c:if>
        
        
        <div class="choose"> 
					<font color="#ffffff"><a href="UserLogin.jsp">back</a></font>
</div>  
		...
		<br>	<br>	
         ...
         <input type="submit" class="submit" name="Submit" value="找回" />   &nbsp;
